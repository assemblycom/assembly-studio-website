          ...
          {section.features.map((feature) => (
            <div key={feature.label}>
              ...
              {feature.href && (
                <Link
                  href={feature.href}
                  className="type-body mt-3 inline-block text-muted-foreground underline underline-offset-4 transition-colors hover:text-foreground"
                >
                  Read the story
                </Link>
              )}
...
export default async function SolutionPage({
  ...
  return (
    <>
      {/* Same centered lede as /security and /templates — the plain light nav
          sits above it, so the page opens on type rather than a hero card. */}
      <section className="px-6 pb-16 pt-24 text-center md:pb-24 md:pt-32">
        <div className="mx-auto max-w-3xl">
          <h1 className="type-display text-balance">{solution.hero.title}</h1>
          <p className="type-lead mx-auto mt-6 max-w-2xl text-pretty text-muted-foreground">
            {solution.hero.description}
          </p>
          <CtaRow
            ctas={solution.hero.ctas}
            className="mx-auto mt-8 sm:justify-center"
          />
        </div>
      </section>
